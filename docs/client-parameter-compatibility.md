...
как top-level поля: например `logprobs`, `audio`, `previous_response_id`,
`container` или `mcp_servers`. Если такой же ключ явно положить внутрь literal
`extra_body`, gpt2giga передаст его в `additional_fields`, а итоговую поддержку
определит GigaChat upstream.
...
сохраненные запросы completions, а также сохраняющие состояние возможности
Responses, такие как `previous_response_id` и `conversation`.
