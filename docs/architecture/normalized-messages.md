...
A trusted external controller owns the provider profile, model route,
`SecretRef`, TLS/proxy policy references, and scoped network ticket. A
credential is resolved and revealed only to the exact
`provider-execution:openai-compatible` boundary; profiles, logs, persisted
settings, network intents, and receipts retain only the reference identity.
Each compatible server must provide an explicitly reviewed profile and
normalized capability/limit contract.
...
smoke requires a remote HTTPS endpoint plus its scoped network grant; this
gateway repository does not expose a public arbitrary-vLLM switch.
...
remain owned by a reviewed external execution boundary. Batches, files,
