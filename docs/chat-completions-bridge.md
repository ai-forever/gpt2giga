...
`ANTHROPIC_AUTH_TOKEN` makes Claude Code send the local gateway key as bearer
authentication. The bridge also accepts `x-api-key`.
...
route. Start Gemini CLI with the alias, not the raw public model name:
...
gateway accepts as its local API key. The custom alias resolves the wire model
to `bridge/chat-only`; the provider profile then resolves it once more to the
...
| Gemini request mentions `topK` or `thinkingConfig` | Select the minimal `chat-bridge` custom alias instead of the raw model or `chat-base`. |
