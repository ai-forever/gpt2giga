...
request-level `preset` override is provided. `gpt2giga/fusion-benchmark` maps
to forced selector benchmark mode, while `gpt2giga/fusion-code` maps to the
verified tool loop preset.
...
Benchmark presets are for simple benchmark/QA validation and intentionally use
`tools_mode="off"`. `verified-tool-loop-ultra` keeps tools enabled across
client-visible tool result rounds and verifies direct native tool calls before
returning them. `code-agent-safe` keeps a full harness envelope and
