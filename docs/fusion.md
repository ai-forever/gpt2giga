...
Specific aliases such as `gpt2giga/fusion-benchmark`,
`gpt2giga/fusion-code`, `gpt2giga/fusion-accuracy` and
`gpt2giga/fusion-code-agent-safe` select the matching built-in preset unless a
request-level `preset` override is provided. `gpt2giga/fusion-benchmark` and
`gpt2giga/fusion-benchmark-tools` map to tool-aware forced selector benchmark
mode, `gpt2giga/fusion-benchmark-text` keeps the text-only forced selector
preset, and `gpt2giga/fusion-code` maps to the verified tool loop preset.
...
- `decision_mode="action"`: direct candidate -> verifier -> action judge. Judge
  возвращает structured `FusionActionDecision`: protocol-level tool call, final
  answer only when task is complete, or blocked action.
...
9. В `action` mode после client tool result adapter возвращается в verified
   loop с включенными tools. Post-tool finalizer используется только для
   `post_tool_mode="finalize"`, unavailable/disabled tools, max tool rounds или
   unsafe repeated tool call.
10. Финальный ответ маппится обратно в OpenAI, Anthropic или Gemini response
...
| `force-benchmark-selector` | `GigaChat-3-Ultra` | `GigaChat-3-Ultra` | yes | `selector` / `minimal` | `solver` | `off` |
| `force-benchmark-selector-tools` | `GigaChat-3-Ultra` | `GigaChat-3-Ultra` | yes | `selector` / `minimal` | `solver` | `schema_only` |
| `verified-tool-loop-ultra` | `GigaChat-3-Ultra` | `GigaChat-3-Ultra` | yes | `action` / `minimal` | `verifier` | `schema_only` |

`GPT2GIGA_FUSION_DEFAULT_PRESET` выбирает preset для aliases и request-level
Fusion configs, где preset не указан.

`force-benchmark-selector` is for text-only benchmark/QA validation and
intentionally uses `tools_mode="off"`; requests with client tools should use
`force-benchmark-selector-tools` or `gpt2giga/fusion-benchmark-tools`.
`verified-tool-loop-ultra` keeps tools enabled across client-visible tool
result rounds and verifies direct native tool calls before returning them.
`code-agent-safe` keeps a full harness envelope and
...
| `GPT2GIGA_FUSION_POST_TOOL_MODE` | `direct_continuation` | Поведение после client tool result: `direct_continuation`, `fusion_continuation`, `verified_continuation` или `finalize`. |
| `GPT2GIGA_FUSION_DIRECT_TOOL_CALL_POLICY` | `return_immediately` | Что делать с валидным native direct tool call до panel stage: вернуть сразу, отправить в `selector` или проверить через `verify_before_return`. |
