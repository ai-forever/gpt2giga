...
export function disposePlaygroundRequestState(state) {
    state.activeRunId += 1;
    state.activeController?.abort();
    state.activeController = null;
}
export async function executePlaygroundRequest(options) {
...
async function consumeStreamResponse(response, request, state, onUpdate, signal) {
    if (!response.body) {
