...
import { buildStoredFileValidationSnapshot, buildUploadValidationMarkup, encodeBytesToBase64, formatApiFormatLabel, } from "./helpers.js";
export function createUploadBindings(deps) {
    const { app, data, elements, page, state, cacheFileRecord, replaceStateForPage, runWorkflowAction, setWorkflowSummary, } = deps;
        ...
        elements.uploadForm?.addEventListener("submit", async (event) => {
            ...
            await runWorkflowAction({
                ...
                action: async () => {
                    ...
                    const mergedResponse = validatedThisSelection && state.uploadValidationReport
                        ? {
                            ...response,
                            validation: buildStoredFileValidationSnapshot(state.uploadValidationReport, state.uploadValidationValidatedAt),
                        }
                        : response;
