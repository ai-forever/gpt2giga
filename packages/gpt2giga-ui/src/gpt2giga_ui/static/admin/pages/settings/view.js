...
import { asArray, asRecord, describeGigachatAuth, describePersistenceStatus, escapeHtml, formatTimestamp, } from "../../utils.js";
import { renderApplicationSection, renderGigachatSection, renderObservabilitySection, renderSecuritySection, } from "../control-plane-sections.js";
import { SETTINGS_LABELS } from "./types.js";
export function renderSettingsContent(state) {
    return state.activeSection === null
        ? renderSettingsHub(state)
        : renderFocusedSettingsPage(state);
}
function renderSettingsHub(state) {
    const activeSinks = asArray(state.observabilityValues.active_sinks);
    ...
    ${card("Persistence posture", `
        <div class="stack">
          <div class="toolbar">
            ...
            ${pill(state.controlPlaneStatus.updated_at
        ...
        ? banner(`Latest revision: ${escapeHtml(asArray(state.revisions[0]?.sections).join(", ") || "no field diff")}.`)
    ...
    ${renderSettingsEntryCard({
        ...
        pills: [
            pill(`Telemetry: ${Boolean(state.observabilityValues.enable_telemetry) ? "on" : "off"}`, Boolean(state.observabilityValues.enable_telemetry) ? "good" : "warn"),
            pill(`Sinks: ${activeSinks.join(", ") || "none"}`),
            pill(`Phoenix: ${asArray(state.observabilityValues.sinks)
    ...
    ${renderSettingsEntryCard({
        ...
        pills: [
            pill(`API key auth: ${Boolean(state.securityValues.enable_api_key_auth) ? "on" : "off"}`, Boolean(state.securityValues.enable_api_key_auth) ? "good" : "warn"),
            pill(`Logs allowlist: ${asArray(state.securityValues.logs_ip_allowlist).length || 0}`),
            pill(`CORS origins: ${asArray(state.securityValues.cors_allow_origins).length || 0}`),
        ],
    })}
    ${card("History", `
        <div class="stack">
...
function renderFocusedSettingsPage(state) {
    ...
    return `
    ${card(`${SETTINGS_LABELS[activeSection]} navigation`, renderSubpageNav({
        currentPage: state.currentPage,
        title: "Settings pages",
...
function renderSettingsMainCard(state) {
    ...
    return card("Recent revisions", state.revisions.length
        ? `
          <div class="stack">
            ${state.revisions
            .map((revision) => {
...
function renderSettingsSidebar(state) {
    const persistence = describePersistenceStatus(state.controlPlaneStatus);
    const gigachatAuth = describeGigachatAuth(state.controlPlaneStatus);
    const detailPills = (() => {
        if (state.activeSection === "application") {
        ...
        if (state.activeSection === "observability") {
            return [
                pill(`Telemetry: ${Boolean(state.observabilityValues.enable_telemetry) ? "on" : "off"}`, Boolean(state.observabilityValues.enable_telemetry) ? "good" : "warn"),
                pill(`Sinks: ${asArray(state.observabilityValues.active_sinks).join(", ") || "none"}`),
        ...
        if (state.activeSection === "security") {
            return [
                pill(`API key auth: ${Boolean(state.securityValues.enable_api_key_auth) ? "on" : "off"}`, Boolean(state.securityValues.enable_api_key_auth) ? "good" : "warn"),
                pill(`Logs allowlist: ${asArray(state.securityValues.logs_ip_allowlist).length || 0}`),
                pill(`Governance rules: ${asArray(state.securityValues.governance_limits).length || 0}`),
            ];
        }
        return [
    ...
    return card(state.activeSection === "history" ? "Rollback posture" : "Section posture", `
      <div class="stack">
        <div class="stack">
