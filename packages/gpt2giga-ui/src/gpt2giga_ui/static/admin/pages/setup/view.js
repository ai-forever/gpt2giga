...
import { asArray, describeGigachatAuth, describePersistenceStatus, escapeHtml, } from "../../utils.js";
import { renderApplicationSection, renderGigachatSection, renderSecuritySection, renderSetupObservabilityHandoff, } from "../control-plane-sections.js";
export function renderSetupContent(state) {
    return state.activeSection === null
        ? renderSetupHub(state)
        : renderFocusedSetupPage(state);
}
function renderSetupHub(state) {
    ...
    ${card("Setup progress", `
        ...
        ? banner(`Bootstrap gate is open. Setup is limited to localhost or the token at ${String(state.bootstrap.token_path ?? "the control-plane volume")}.`, "warn")
    ...
    ${card("Next recommended step", `
        <div class="stack">
          ...
          <div class="toolbar">
            ${pill(`Observability sinks: ${asArray(state.observabilityValues.active_sinks).join(", ") || "none"}`)}
...
function renderFocusedSetupPage(state) {
    return `
...
function renderSetupMainCard(state) {
    if (state.activeSection === "claim") {
        return card("Claim instance", `
        <div class="stack">
          ${banner(state.claim.required
            ? state.claim.claimed
            ...
            : "Claiming is not required.", state.claim.claimed ? "info" : "warn")}
          ${state.claim.claimed
            ? `
                  <div class="dual-grid">
                    <div class="stack">
...
function renderSetupSidebar(state) {
    if (state.activeSection === "security") {
      ...
      ${card("Observability handoff", renderSetupObservabilityHandoff(state.observabilityValues), "panel panel--span-4 panel--aside")}
      ${renderSetupStatusCard(state)}
    `;
    }
    return renderSetupStatusCard(state);
}
function renderSetupStatusCard(state) {
    const persistence = describePersistenceStatus(state.setup);
    const gigachatAuth = describeGigachatAuth(state.setup);
