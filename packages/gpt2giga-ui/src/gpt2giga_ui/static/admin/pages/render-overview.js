...
import { banner, card, kpi, pill, renderDefinitionList, renderGuideLinks, renderPageFrame, renderPageSection, renderStatLines, renderTable, } from "../templates.js";
      ...
      `,
        ...
        sections: [
            renderPageSection({
                ...
                description: "Keep the first screen focused on posture, active failures, and the next operator surface.",
            ...
            ${card("Recent issues", `
                <div class="stack">
                  ${banner(errorCount > 0
                    ...
                    : "No recent errors are visible in the current rollup.", errorCount > 0 ? "warn" : "info")}
                  ${renderTable([
                    { label: "When" },
                    { label: "Route" },
                    { label: "Failure" },
              ...
              `, "panel panel--span-8 panel--measure")}
            ${card("Provider snapshot", renderDefinitionList(buildProviderSnapshot(providerEntries, topProvider, enabledProviders, requestCount, totalTokens), "Provider snapshot is unavailable."), "panel panel--span-4 panel--aside")}
          `,
            }),
            renderPageSection({
                eyebrow: "Handoff",
                title: "Surface map and secondary diagnostics",
                description: "Keep guides and broader posture below the dashboard so the first screen stays operational.",
                bodyClassName: "page-grid",
                body: `
            ${card("Surface handoff", renderDefinitionList(buildSurfaceHandoffs(setup, requestCount, errorCount, enabledProviders, topProvider, docsEnabled), "Surface handoff is unavailable."), "panel panel--span-4 panel--aside")}
            ${card("Gateway posture", renderDefinitionList([
                    {
                        label: "Runtime mode",
                        value: String(runtime.mode ?? "n/a"),
                        note: docsEnabled
                ...
                ], "Gateway posture is unavailable."), "panel panel--span-4 panel--aside")}
            ${card("Operator guides", renderGuideLinks([
                    {
...
function describeNextSurface(setup, errorCount, requestCount) {
...
function buildProviderSnapshot(providerEntries, topProvider, enabledProviders, requestCount, totalTokens) {
    ...
    }).concat(topProvider && enabledProviders.length > providerEntries.length
        ? [
            {
...
function buildSurfaceHandoffs(setup, requestCount, errorCount, enabledProviders, topProvider, docsEnabled) {
    return [
        {
            label: "Playground",
