...
Qwen Code использует конфигурацию провайдеров моделей в JSON (`~/.qwen/settings.json`). Ниже приведён готовый пример блока, который можно вставить в существующий файл настроек.
...
> `gpt2giga` поддерживает как корневые OpenAI-совместимые маршруты, так и
> маршруты с префиксами `/v1` и `/v2`. Корневой `http://localhost:8090`
> следует `GPT2GIGA_GIGACHAT_API_MODE=v1|v2`; `/v1` всегда идёт в GigaChat v1
> contract, `/v2` — в GigaChat v2 contract.
