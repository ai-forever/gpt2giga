...
### Исправлено
- **Параллельные function calls в GigaChat v2 SDK**: обновлённый vendored-wheel `gigachat==0.2.4a1` сериализует `model_options.parallel_tool_calls` и сохраняет `id` каждого `function_call` в ответе. Legacy v1-контракт не изменён.
...
- **Владение OpenAI Responses**: native GigaChat Responses остаётся стабильным compatibility path для hosted tools, attachments, state, v1/v2 и SSE. Route-selected normalized Responses имеет статус `technical_preview`, пока accepted corpus не докажет native parity.
...
- **Model inventory**: модели GigaChat обнаруживаются динамически; provider-profile v2 отличает `model_inventory: dynamic` от опциональных точных aliases и static profiles внешних providers.
...
### Добавлено
- **OpenAI-compatible Files API**: подключены проверенные маршруты загрузки, списка, получения метаданных, скачивания содержимого и удаления файлов независимо от всё ещё отключённого Batches API.
- **Передача вложений из Harness**: доверенные локальные запросы Harness могут привязать к текущему пользовательскому сообщению OpenAI Responses ограниченный набор проверенных идентификаторов файлов GigaChat.

### Исправлено
- **Загрузка документов**: файлы теперь отправляются с универсальным назначением GigaChat `general` и без несовместимого content-type в multipart tuple, поэтому поддерживаемые PDF и другие документы доходят до провайдера.
- **Сохранение вложений**: существующие идентификаторы вложений сохраняются и дедуплицируются при преобразовании составного текстового, графического и файлового контента.
...
- **Tool schema normalization**: function/tool schemas теперь нормализуются через общий helper, получают object root и `properties` по умолчанию, а `allOf` схлопывается в поддерживаемую GigaChat schema форму.
- **GigaChat v2 stream completion**: stream adapter корректно разбирает raw SSE strings, wrapped `data`, named done events, `choices[].delta/message`, `[DONE]` и usage aliases `input/output` или `prompt/completion`.
- **Source rendering**: GigaChat source markers и `inline_data.sources` теперь превращаются в видимый Markdown `Sources` appendix в OpenAI Chat, OpenAI Responses и streaming flows; Responses annotations используют raw text, чтобы ссылки не терялись после удаления markers.
...
### Добавлено
- **Явные GigaChat API mode routes**: публичные OpenAI/Anthropic/LiteLLM-compatible routes теперь доступны в корне, под `/v1` и под `/v2`; root routes следуют `GPT2GIGA_GIGACHAT_API_MODE`, `/v1` принудительно выбирает legacy chat contract, `/v2` - GigaChat chat-completion contract.
...
### Добавлено
- **GigaChat v2 backend mode**: добавлен `GPT2GIGA_GIGACHAT_API_MODE` для переключения chat-like upstream-вызовов на GigaChat chat-completion contract (`v2/chat/completions` в SDK 0.2.2a1); внешний OpenAI/Anthropic-compatible контракт и URL остаются прежними
