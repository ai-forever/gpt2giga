...
Схема `gpt2giga.provider-profiles.v3` добавляет явные возможности модели и
токенные лимиты, необходимые для исполнения алиаса `openai_compatible`. Она
также разрешает намеренно открытый профиль без ключа. Для upstream только с
Chat Completions используйте v3; файлы v1/v2 продолжают загружаться, но сами по
себе не включают такой маршрут исполнения.
