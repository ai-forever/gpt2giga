...
Responses, Anthropic Messages и Gemini GenerateContent также могут выбрать
точный алиас из профиля `openai_compatible`. Эти три ячейки имеют статус
`technical_preview` и используют один созданный при запуске адаптер Chat
Completions. Прямая ячейка OpenAI Chat Completions остаётся заблокированной,
пока её публичный маршрут не подключён к этому реестру. Профиль v3 обязан
объявить проверенные возможности и лимиты модели. Неподдерживаемая семантика и
запросы точного подсчёта токенов отклоняются до сетевого вызова.
