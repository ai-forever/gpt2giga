...
- либо Chat Completions SSE с завершающим кадром `data: [DONE]`, либо явно
...
этого отправляет `stream: false` и использует usage из полного JSON-ответа.
Если сервер не вернул usage, gpt2giga не придумывает значения токенов.
...
### Буферизованные ответы upstream

Некоторые OpenAI-совместимые серверы принимают `stream: true`, но всё равно
возвращают один полный Chat Completion с типом `application/json`. Для такого
сервера явно включите отложенный синтез потока в профиле v3:

```yaml
    upstream_stream_mode: buffered
```

Шлюз отправит upstream-параметр `stream: false`, проверит полный ответ и выдаст
клиенту одну текстовую дельту или по одной полной дельте на function call,
после чего передаст завершающее событие и usage. Этого достаточно для обычных
циклов с инструментами в Codex, Claude Code и Gemini CLI.

Буферизованный режим не даёт поток токенов от модели. Первое событие с текстом
или function call приходит только после завершения полного ответа, поэтому в
...
```yaml
    ...
    upstream_stream_mode: buffered  # только если у сервера нет настоящего SSE
```
