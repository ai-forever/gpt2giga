...
| `upstream_stream_mode` | Только для OpenAI-compatible профиля v3. Без поля или со значением `sse` требуется строгий upstream SSE. Значение `buffered` отправляет upstream `stream: false` и синтезирует отложенный поток из полного JSON-ответа. |
...
Также v3 разрешает профиль upstream без ключа и явный буферизованный синтез
потока для OpenAI-compatible сервера без настоящего SSE. Статическому профилю
по-прежнему нужен хотя бы один алиас.
