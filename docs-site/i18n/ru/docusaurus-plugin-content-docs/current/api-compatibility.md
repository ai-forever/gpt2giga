...
доступен, а иначе определяет embedding-модели по их идентификатору. Потребители
могут использовать эти метаданные, чтобы не показывать embedding-only модели в
