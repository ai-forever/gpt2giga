    ...
    def query(
        ...
        *,
        ...
        if not filters:
            return items
        return [
            item for item in items if _mapping_matches_filters(item, filters=filters)
        ]
        ...
        *,
        ...
        if extra_filters:
            items = [
                item
                for item in items
                if _mapping_matches_filters(item, filters=extra_filters)
            ]
        items.reverse()
        return items
...
def _mapping_matches_filters(item: Any, *, filters: Mapping[str, Any]) -> bool:
    """Return whether a mapping-like item matches all equality filters."""
