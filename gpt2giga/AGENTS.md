...
- Chat flow:
  `api/openai/chat.py` -> `features/chat/service.py` -> `providers/gigachat/chat_mapper.py` -> `RequestTransformer.prepare_chat_completion(...)` or `prepare_chat_completion_v2(...)` -> GigaChat `achat/astream` or `achat.create/achat.stream` -> `ResponseProcessor`
- Responses flow:
  `api/openai/responses.py` -> `features/responses/service.py` -> `RequestTransformer.prepare_response(...)` or `prepare_response_v2(...)` -> GigaChat `achat` or `achat.create` -> `ResponseProcessor.process_response_api(...)` or `process_response_api_v2(...)`
