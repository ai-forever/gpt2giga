...
export function bindSetupPage(app: AdminApp, state: SetupPageState): void {
  const claimForm = app.pageContent.querySelector<HTMLFormElement>("#setup-claim-form");
  const applicationForm = app.pageContent.querySelector<HTMLFormElement>(
    "#setup-application-form",
  );
  const gigachatForm = app.pageContent.querySelector<HTMLFormElement>(
    "#setup-gigachat-form",
  );
  const securityForm = app.pageContent.querySelector<HTMLFormElement>(
    "#setup-security-form",
  );
  const applicationStatusNode = app.pageContent.querySelector<HTMLElement>(
    "#setup-application-status",
  );
  const gigachatStatusNode = app.pageContent.querySelector<HTMLElement>(
    "#setup-gigachat-status",
  );
  const securityStatusNode = app.pageContent.querySelector<HTMLElement>(
  ...
  if (applicationForm && applicationStatusNode) {
    bindControlPlaneSectionForm({
      ...
      buildEntries: (payload) =>
        buildPendingDiffEntries("application", state.applicationValues, payload),
      buildNote: () =>
  ...
  if (gigachatForm && gigachatStatusNode) {
    ...
    const gigachatBinding = bindControlPlaneSectionForm({
      ...
      buildEntries: (payload) =>
        buildPendingDiffEntries("gigachat", state.gigachatValues, payload),
      buildNote: () => {
  ...
  if (securityForm && securityStatusNode) {
    bindControlPlaneSectionForm({
      ...
      buildEntries: (payload) =>
        buildPendingDiffEntries("security", state.securityValues, payload),
      buildNote: () =>
