...
export async function renderOverview(app: AdminApp, token: number): Promise<void> {
  ...
  const recentErrors = asArray<Record<string, unknown>>(errors.events)
    .slice(-5)
    .reverse();
  const enabledProviders = asArray<string>(runtime.enabled_providers).join(", ") || "none";
  ...
  app.setContent(
    [
      ...
      card(
        "Recent errors",
        recentErrors.length ? renderJson(recentErrors) : `<p>No recent errors recorded.</p>`,
        "panel panel--span-4",
      ),
      card("Usage by key", renderJson(usageKeys.entries ?? []), "panel panel--span-6"),
      card(
        "Usage by provider",
        renderJson(usageProviders.entries ?? []),
        "panel panel--span-6",
      ),
    ].join(""),
  );
