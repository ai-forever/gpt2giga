...
export async function renderLogs(app: AdminApp, token: number): Promise<void> {
  ...
  const logLines = app.pageContent.querySelector<HTMLInputElement>("#log-lines");
  ...
  const refreshLogs = async () => {
    logOutput.textContent = await app.api.text(
      `/admin/api/logs?lines=${encodeURIComponent(logLines.value || "150")}`,
    );
  };
