...
type SurfaceId =
  ...
  | "anthropic-messages"
  | "gemini-generate";
type RunPhase = "idle" | "sending" | "streaming" | "success" | "error" | "aborted";

interface AdminAppLike {
  readonly pageContent: HTMLElement;
  api: {
    json<T>(path: string, init?: unknown, useGateway?: boolean): Promise<T>;
  };
...
export async function renderPlayground(app: AdminAppLike, token: number): Promise<void> {
  ...
  const runState: PlaygroundRunState = createIdleRunState();
  const streamEvents: ParsedSseEvent[] = [];
  ...
  const requestSummary = app.pageContent.querySelector<HTMLElement>("#playground-request-summary");
  const requestBody = app.pageContent.querySelector<HTMLPreElement>("#playground-request-body");
  const bootstrapBanner = app.pageContent.querySelector<HTMLElement>("#playground-bootstrap-banner");
  const bootstrapSummary = app.pageContent.querySelector<HTMLElement>("#playground-bootstrap-summary");
  const authNote = app.pageContent.querySelector<HTMLElement>("#playground-auth-note");
  const formNote = app.pageContent.querySelector<HTMLElement>("#playground-form-note");
  const runNote = app.pageContent.querySelector<HTMLElement>("#playground-run-note");
  const runSummary = app.pageContent.querySelector<HTMLElement>("#playground-run-summary");
  const statusPill = app.pageContent.querySelector<HTMLElement>("#playground-status-pill");
  const assistantOutput = app.pageContent.querySelector<HTMLPreElement>(
    "#playground-assistant-output",
  );
  const outputMeta = app.pageContent.querySelector<HTMLElement>("#playground-output-meta");
  const transportMeta = app.pageContent.querySelector<HTMLElement>("#playground-transport-meta");
  const output = app.pageContent.querySelector<HTMLPreElement>("#playground-output");
  const stopButton = document.getElementById("playground-stop") as HTMLButtonElement | null;
  const resetButton = document.getElementById("playground-reset") as HTMLButtonElement | null;
  const submitButton = app.pageContent.querySelector<HTMLButtonElement>("#playground-submit");
  const gatewayKeyInput = document.getElementById("gateway-key-input") as HTMLInputElement | null;
  const presetButtons = Array.from(
    app.pageContent.querySelectorAll<HTMLButtonElement>(".playground-preset"),
  );
  ...
  const updateRequestPreview = (): void => {
  ...
  const updateRunPanels = (): void => {
    statusPill.innerHTML = renderPhasePill(runState.phase);
    outputMeta.innerHTML = pill(
      runState.assistantOutput.trim() ? "parsed" : runState.phase === "error" ? "error" : "waiting",
      ...
      { label: "Status", value: formatStatus(runState.statusCode, runState.statusText) },
      {
        ...
        value:
          runState.startedAt === null
            ? "0 ms"
            : formatDurationMs(
                (runState.finishedAt ?? performance.now()) - runState.startedAt,
              ),
      },
  ...
  await readSseStream(
    response.body,
    {
      ...
      onEvent: (event) => {
        ...
        runState.assistantOutput = mergeAssistantOutput(
          runState.assistantOutput,
          textDelta,
          event.type,
          payload,
...
function applyPreset(form: HTMLFormElement, preset: PlaygroundPreset): void {
  const fields = getFields(form);
...
function getFields(form: HTMLFormElement): FormFields {
  return form.elements as FormFields;
}

function createIdleRunState(): PlaygroundRunState {
  return {
    phase: "idle",
    note: DEFAULT_OUTPUT,
    request: null,
...
function buildRequestFromPreset(preset: PlaygroundPreset): PlaygroundRequest {
  return buildRequestFromState({
...
function buildGatewayHeaders(surface: SurfaceId, gatewayKey: string): Headers {
...
function buildBootstrapSteps(setup: SetupPayload, gatewayKey: string): Array<{
  label: string;
  value: string;
  note?: string;
}> {
...
function extractAssistantText(payload: unknown, surface: SurfaceId): string {
...
function formatSseTranscript(events: ParsedSseEvent[]): string {
  if (!events.length) {
