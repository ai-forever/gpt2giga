  ...
  app.setContent(`
    ...
    ${card(
      ...
      `
        <div class="stack">
          ...
          ${renderSetupSteps(
            asArray<Record<string, unknown>>(setup.wizard_steps).map((step) => ({
              id: String(step.id ?? ""),
    ...
    ${card(
      ...
      renderStatLines(
        ...
        "Mounted route summary is unavailable.",
      ),
      "panel panel--span-3",
    )}
    ${card(
      ...
      renderStatLines(
        [
          ...
          },
          {
            ...
            value: asArray<unknown>(runtime.enabled_providers).map(String).join(", ") || "none",
          },
          {
            label: "Telemetry",
...
function summarizeRoutes(routes: RouteRow[]): Record<string, number> {
  const counts = {
    admin: 0,
    openai: 0,
    anthropic: 0,
