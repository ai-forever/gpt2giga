...
import {
  ...
  type ObservabilityFormFields,
  type ObservabilityPresetDescriptor,
  type ObservabilitySectionOptions,
  type SectionValues,
} from "./types.js";

interface SetupObservabilitySummary {
  message: string;
  tone: "info" | "warn";
}
    ...
    <div class="stack">
      <div class="pill-row">
        ${pill(
          ...
          state.telemetryEnabled ? "good" : "warn",
        )}
        ${pill(`Active sinks: ${state.activeSinkCount}`, state.activeSinkCount ? "good" : "warn")}
        ${pill(
          state.metricsEnabled ? "Metrics endpoint: live" : "Metrics endpoint: disabled",
          state.metricsEnabled ? "good" : "warn",
        )}
      </div>
...
export function buildSetupObservabilityHandoffState(values: SectionValues): {
  activeSinkCount: number;
  metricsEnabled: boolean;
  summaries: SetupObservabilitySummary[];
...
export function describeOtlpHeaderPreview(otlp: SectionValues): string {
...
function describeSetupObservabilitySink(
  sink: Record<string, unknown>,
): SetupObservabilitySummary {
