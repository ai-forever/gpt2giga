...
import {
  buildLogEventSelectionSummary,
  buildLogsUrl,
  buildStreamDiagnostics,
  buildTailContextRows,
  buildTailSelectionSummary,
  countMatchingLines,
  describeStreamPhase,
...
import type { LogsPageElements } from "./view.js";

interface BindLogsPageOptions {
  app: AdminApp;
  data: LogsPageData;
  elements: LogsPageElements;
  filters: LogsFilters;
  token: number;
}

export function bindLogsPage(options: BindLogsPageOptions): void {
  ...
  const inspectPayloads: Record<LogSelectionKind, LogEvent[]> = {
  ...
  const setSelectionFromEvent = (kind: LogSelectionKind, item: LogEvent): void => {
    ...
    const counterpart =
      requestId && kind === "request"
        ...
        : requestId
          ...
          : null;
    elements.summaryNode.innerHTML = renderDefinitionList(
  ...
  const renderStreamDiagnosticsPanel = (): void => {
    elements.streamDiagnostics.innerHTML = renderDefinitionList(
