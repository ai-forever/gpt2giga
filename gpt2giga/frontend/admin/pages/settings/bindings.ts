...
export function bindSettingsPage(app: AdminApp, state: SettingsPageState): void {
...
function bindSettingsForms(options: {
  ...
  const {
    ...
    syncPhoenixApiKey,
  } = bindObservabilitySecretFields(observabilityForm, options.state.observabilityValues);
  ...
  if (applicationForm && applicationStatusNode) {
    bindControlPlaneSectionForm({
      ...
      buildEntries: (payload) =>
        buildPendingDiffEntries("application", options.state.applicationValues, payload),
      buildNote: () =>
  ...
  if (observabilityForm && observabilityStatusNode) {
    const observabilityBinding = bindControlPlaneSectionForm({
      ...
      buildEntries: (payload) =>
        buildObservabilityDiffEntries(options.state.observabilityValues, payload),
      buildNote: () => {
  ...
  if (gigachatForm && gigachatStatusNode) {
    ...
    const gigachatBinding = bindControlPlaneSectionForm({
      ...
      buildEntries: (payload) =>
        buildPendingDiffEntries("gigachat", options.state.gigachatValues, payload),
      buildNote: () => {
  ...
  if (securityForm && securityStatusNode) {
    bindControlPlaneSectionForm({
      ...
      buildEntries: (payload) =>
        buildPendingDiffEntries("security", options.state.securityValues, payload),
      buildNote: () =>
        "Restart-sensitive controls are flagged before save.",
      getValidationMessage: (payload, report = false) =>
        validateJsonArrayField(
