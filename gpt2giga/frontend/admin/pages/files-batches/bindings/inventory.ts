...
import type { BatchComposerBindings } from "./batch-composer.js";
import {
  buildBatchOutputSelectionSurface,
  buildBatchSelectionSurface,
  buildFileSelectionSurface,
  resolveContentPathForFile,
  resolveDownloadFilename,
  resolveDownloadPathForFile,
  resolvePreviewBytes,
} from "./inventory-selection.js";

interface InventoryBindingsDeps {
  app: AdminApp;
  ...
  batchComposer: BatchComposerBindings;
  cacheFileRecord: (payload: FileRecord) => FileRecord;
  cacheBatchRecord: (payload: BatchRecord) => BatchRecord;
  removeFileRecord: (fileId: string) => FileRecord | null;
  setSummary: (items: DefinitionItem[]) => void;
...
export function createInventoryBindings(
  deps: InventoryBindingsDeps,
): InventoryBindings {
  const {
    app,
    data,
  ...
  const applyFileSelectionSurfaces = (
    ...
    const validationSnapshot = batchComposer.resolveDisplayedFileValidationSnapshot(
      fileId,
      source,
    );
  ...
  const refreshSelectedFileValidationSurface = (): void => {
    ...
    const mode =
      elements.batchInput?.value.trim() === state.selection.fileId
        ? "composer"
        : "inspect";
      ...
      action: async () => {
        ...
        if (preview.handoffRequestId && options?.relatedBatch) {
          state.selection = {
            ...
            inputFileId:
              String(
                options.relatedBatch.input_file_id ?? state.selection.inputFileId ?? "",
              ) || undefined,
            outputFileId: fileId,
  ...
  const bindInspectorActions = (): void => {
    elements.actionNode.addEventListener("click", async (event) => {
      ...
      if (action === "download-file" && state.selection.fileId) {
        await downloadFileContent(
          ...
          resolveDownloadFilename(
            state.selection.fileId,
            inventory.fileLookup.get(state.selection.fileId),
          ),
          button,
        );
        return;
      ...
      if (action === "preview-linked-output" && state.selection.fileId) {
        const latestOutputBatch = getLatestOutputBatch(
          state.selection.fileId,
          data.batches,
        );
  ...
  const bindPageInventoryActions = (): void => {
    ...
    app.pageContent
      .querySelectorAll<HTMLElement>("[data-file-content]")
      .forEach((item) => {
        item.addEventListener("click", async () => {
    ...
    app.pageContent
      .querySelectorAll<HTMLElement>("[data-file-download]")
      .forEach((item) => {
        item.addEventListener("click", async () => {
          ...
          const filename =
            item.dataset.fileDownloadName?.trim() ||
    ...
    app.pageContent
      .querySelectorAll<HTMLElement>("[data-batch-output]")
      .forEach((item) => {
        item.addEventListener("click", async () => {
    ...
    app.pageContent
      .querySelectorAll<HTMLElement>("[data-batch-download]")
      .forEach((item) => {
        item.addEventListener("click", async () => {
          ...
          const filename =
            item.dataset.batchDownloadName?.trim() ||
