...
export interface SelectionSurface {
  summary: DefinitionItem[];
  detailTitle: string;
  detailItems: DefinitionItem[];
  detailPayload: string;
  detailOpen: boolean;
}

export function findBatchByOutputFileId(
  fileId: string,
  batches: BatchRecord[],
): BatchRecord | null {
  return (
    batches.find((entry) => String(entry.output_file_id ?? "") === fileId) ?? null
  );
}
...
export function resolveDownloadPathForFile(options: {
  ...
  return (
    findBatchByOutputFileId(fileId, batches)?.output_path?.trim() ||
...
export function buildFileSelectionSurface(options: {
  ...
  detailPayload: string;
  batches: BatchRecord[];
  validationSnapshot: FileValidationSnapshot | null;
}): SelectionSurface {
...
export function buildBatchOutputSelectionSurface(options: {
  ...
  variant: "batch-output" | "latest-linked-output";
}): SelectionSurface {
