...
export function buildRequestFromPreset(preset: PlaygroundPreset): PlaygroundRequest {
  return buildRequestFromState({
...
export function applyPlainResponse(
  rawText: string,
...
export function extractAssistantText(payload: unknown, surface: SurfaceId): string {
  const record = asRecord(payload);
...
export function mergeAssistantOutput(
  current: string,
  next: string,
  eventType: string,
  payload: unknown,
