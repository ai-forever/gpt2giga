...
export async function renderProviders(app: AdminApp, token: number): Promise<void> {
  ...
  app.setContent(
    renderPageFrame({
      toolbar: renderProvidersToolbar(
        enabledProviders,
        routeRows.length,
              ...
              `
                <div class="stack">
                  ${renderTable(
                    ...
                    inventoryRows,
                    "No provider rows were reported by the admin API.",
                  )}
                </div>
              `,
...
function buildProviderWarnings(
  ...
  if (enabledProviderCount === 0) {
    warnings.push(
      banner(
        "No provider surface is enabled. Admin and System can mount, but client traffic has no provider path.",
        "warn",
      ),
    );
  }
  if (!backend.telemetry_enabled) {
    warnings.push(
      banner(
        "Telemetry is disabled. Provider posture stays config-only until raw logs or traffic.",
        "warn",
      ),
    );
  }
  if (!backend.governance_enabled) {
    warnings.push(
      banner(
        "Governance limits are disabled. Throughput is visible, but requests are not constrained.",
        "info",
      ),
    );
  }
...
function renderProvidersToolbar(
...
function buildProviderInventoryRows(rows: ProviderRow[]): string[][] {
  return rows.map((row) => {
    const capabilitiesList = readStringList(row.capabilities);
    const routesList = readStringList(row.routes);
    ...
    return [
      ...
      `<strong>${escapeHtml(enabled ? "Smoke in playground" : "Enable in Settings")}</strong><br /><span class="muted">${escapeHtml(
        enabled ? "Validate request flow in Playground or Traffic." : "Restore auth and provider posture first.",
      )}</span>`,
      `
        <div class="toolbar">
