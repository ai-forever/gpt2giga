    ...
    ${card(
      ...
      `
        <div class="step-grid">
          ${surfaceRows
            .map((row) => {
              const capabilitiesList = asArray<unknown>(row.capabilities).map(String);
              const routesList = asArray<unknown>(row.routes).map(String);
              ...
              return `
                <article class="step-card ${enabled ? "step-card--ready" : ""}">
                  ...
                  <p class="muted">${escapeHtml(
                    `${formatNumber(row.route_count ?? routesList.length)} routes`,
                  )}</p>
                </article>
              `;
            })
