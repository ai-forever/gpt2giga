...
export function renderTrafficInspector(options: TrafficInspectorOptions): string {
  return `
...
export function renderRequestPreviewRows(
...
export function renderErrorPreviewRows(
  events: Record<string, unknown>[],
  filters: TrafficFilters,
): string {
  if (!events.length) {
...
export function renderUsagePreviewRows(entries: Record<string, unknown>[]): string {
  if (!entries.length) {
