...
import { card, kpi, pill, renderDefinitionList, renderFilterSelectOptions, renderStaticSelectOptions, } from "../templates.js";
import { asArray, escapeHtml, formatBytes, formatTimestamp, safeJsonParse, setQueryParamIfPresent, } from "../utils.js";
...
export async function renderFilesBatches(app, token) {
  ...
  `);
    ...
    const mediaNode = app.pageContent.querySelector("#files-batches-media");
    const summaryNode = app.pageContent.querySelector("#files-batches-summary");
    const workflowNode = app.pageContent.querySelector("#files-batches-workflow");
    ...
    uploadForm.addEventListener("submit", async (event) => {
        ...
        await runWorkflowAction({
            ...
            action: async () => {
                ...
                window.history.replaceState({}, "", buildFilesBatchesUrl(filters, {
                    ...routeState,
    ...
    app.pageContent.querySelectorAll("[data-batch-input-preview]").forEach((item) => {
        item.addEventListener("click", async () => {
...
function buildIdleWorkflowSummary() {
    return [
...
function renderInspectorActions(selection, fileLookup, batchLookup, batches) {
...
function buildContentPreviewSummary(preview, fileId, label, options) {
    const summary = [
...
function readFilesBatchesRouteState() {
    const params = new URLSearchParams(window.location.search);
    return {
...
function buildFilesBatchesUrl(filters, routeState) {
...
function summarizePreviewOutcome(preview) {
    return [
        preview.formatLabel,
        preview.contentKind,
        preview.kind === "image"
...
function buildFilePreview(bytes, filename) {
...
function getLinkedBatchesForFile(fileId, batches) {
    return batches
        .filter((batch) => {
        const inputFileId = String(batch.input_file_id ?? "");
...
function getLatestLinkedBatch(fileId, batches) {
    return getLinkedBatchesForFile(fileId, batches)[0] ?? null;
}
function getLatestOutputBatch(fileId, batches) {
    return (getLinkedBatchesForFile(fileId, batches).find((batch) => Boolean(String(batch.output_file_id ?? ""))) ??
        null);
}
function summarizeBatchRequestCounts(value) {
...
function buildBatchActionHint(batch) {
    if (!batch) {
...
function isBatchInputRow(value) {
    return Boolean(value &&
        typeof value === "object" &&
        ("body" in value || "request" in value));
}
